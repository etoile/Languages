...
IoState *self = ((IoObject *)context)->tag->state;
	...
	s = "Addon := Object clone do(\n"
  	...
  	"        ioFiles foreach(file, IoVM doFile(file path))\n"
  	"        Lobby getSlot(name)\n"
  	"    )\n"
  	"\n"
  	...
  	"        addonFolders := searchFolders map(folders) flatten select(fileNames contains(\"build.io\"))\n"
	...
	s = "\n"
  	...
  	")\n"
  	"\n"
  	"Map serialized := method(b,\n"
  	"	if(b == nil, b := Sequence clone)\n"
  	"	b appendSeq(\"Map clone do(\")\n"
  	"	self foreach(k, v,\n"
  	"		b appendSeq(\"atPut(\" .. k serialized .. \", \" .. v serialized .. \");\")\n"
  	"	)\n"
  	"	b appendSeq(\");\")\n"
  	")\n"
  	"\n"
