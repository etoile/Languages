...
IoState *self = ((IoObject *)context)->tag->state;
	...
	s = "true setSlot(\"and\", Object getSlot(\"evalArg\"))\n"
  	...
  	"	)\n"
  	"\n"
  	"        //setSlot(\"opShuffle\", getSlot(\"opShuffleIo\"))\n"
	...
	s = "Addon := Object clone do(\n"
  	...
  	"    dllSuffix := method(if(platform == \"darwin\", \"dylib\", \"so\"))\n"
  	...
  	"    \n"
  	"    ioFiles := method(Directory with(addonPath) folderNamed(\"io\") files select(path endsWithSeq(\".io\")))\n"
  	"\n"
  	"    dependencies := method(\n"
  	...
  	"\n"
  	"    load := method(\n"
  	"        //writeln(\"Addon \", name, \" loading\")\n"
  	"        loadDependencies\n"
  	"        //writeln(dllPath)\n"
  	"        DynLib clone setPath(dllPath) open call(\"Io\" .. name .. \"Init\", IoVM)\n"
  	"        ioFiles foreach(file, Lobby doFile(file path))\n"
  	"        Lobby getSlot(name)\n"
  	"    )\n"
  	"\n"
  	...
  	"    debug := false\n"
  	"\n"
  	"    searchPaths := list(\".\", \"/usr/local/lib/io/addons\")\n"
  	"    appendSearchPath := method(p, searchPaths appendIfAbsent(p); self)\n"
  	"\n"
  	"    addonFor := method(name, searchPaths map(path, Addon clone setRootPath(path) setName(name)) detect(exists))\n"
	...
	IoState_rawOn_doCString_withLabel_(self, context, s, "io/P_CGI.io");
	...
	s = "Range do(\n"
  	...
  	"	slice := method(s, e, b,\n"
  	"		if(e < s, Exception raise(\"Starting point must be greater than the ending point.\"))\n"
  	"		if(b isNil, b = 1)\n"
  	"		l := list\n"
	...
	s = "\n"
  	...
  	"	if(b isNil, b = Sequence clone)\n"
  	"	b appendSeq(self asMutable escape asSimpleString)\n"
  	")\n"
  	"\n"
