...
void IoVMCodeInit(IoState *self, IoObject *context)
{
	...
	s = "/*\n"
  	...
  	"    	getSlot(\"self\") slotNames sortInPlace foreach(slotName,\n"
  	"    		//s appendSeq(self getSlot(slotName) asSimpleString)\n"
  	"                s appendSeq(\"  \", slotName alignLeft(16), \" = \", getSlot(\"self\") getSlot(slotName) asSimpleString ifNilEval(\"?\"), \"\\n\")\n"
  	"    	)\n"
  	"    	s\n"
  	"    )\n"
  	"    \n"
	...
	s = "\n"
  	...
  	"  List flatten := method(\n"
  	"    l := List clone\n"
  	"    self foreach(v, if(v type == \"List\", l appendSeq(v flatten), l append(v)))\n"
  	"    l\n"
  	"  )\n"
  	"     \n"
	...
	s = "Sequence do(\n"
  	...
  	"		coro raiseException(self clone setError(error) setCoroutine(coro))\n"
  	"	)\n"
  	"	\n"
  	...
  	"\n"
  	"	showStack := method(coroutine showStack)\n"
  	")\n"
  	"";
	...
	s = "\n"
  	"nil   serialized := method(\"nil\")\n"
  	"true  serialized := method(\"true\")\n"
  	"false serialized := method(\"false\")\n"
  	"\n"
  	"Sequence serialized := method(self asMutable replaceSeq(\"\\\"\", \"\\\\\\\"\") asSimpleString)\n"
  	"\n"
  	"Number serialized := method(self asSimpleString)\n"
  	"\n"
  	"List serialized := method(\"list(\" .. self map(v, getSlot(\"v\") serialized) join(\", \") .. \");\")\n"
  	"\n"
  	"Object serialized := method(\n"
  	"	s := Sequence clone\n"
  	"	s appendSeq(self type, \" clone do(\\n\") \n"
  	"	self slotNames foreach(slotName, \n"
  	"		s appendSeq(\"\\t\", slotName, \" := \", self getSlot(slotName) serialized, \"\\n\")\n"
  	"	)\n"
  	"	s appendSeq(\")\\n\")\n"
  	")";
